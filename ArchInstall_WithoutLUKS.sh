...
### SSD Filesystem ###
echo -e "\n\033[1;34mCreating Btrfs filesystem\033[0m"
mkfs.btrfs -f -L ARCH "/dev/${SSD}p2"
mount "/dev/${SSD}p2" /mnt
...
# Mounting SSD
mount -o noatime,compress=zstd,space_cache=v2,subvol=@ "/dev/${SSD}p2" /mnt
mkdir -p /mnt/{boot/efi,home,.snapshots,var/log}
mount -o noatime,compress=zstd,space_cache=v2,subvol=@home "/dev/${SSD}p2" /mnt/home
mount -o noatime,compress=zstd,space_cache=v2,subvol=@snapshots "/dev/${SSD}p2" /mnt/.snapshots
mount -o noatime,compress=zstd,space_cache=v2,subvol=@var_log "/dev/${SSD}p2" /mnt/var/log
...
{
    echo -e "\n# Btrfs optimizations"
    echo "/dev/${SSD}p2 / btrfs rw,noatime,compress=zstd,space_cache=v2,subvol=@ 0 0"
    echo "/dev/${SSD}p2 /home btrfs rw,noatime,compress=zstd,space_cache=v2,subvol=@home 0 0"
    echo "/dev/${SSD}p2 /.snapshots btrfs rw,noatime,compress=zstd,space_cache=v2,subvol=@snapshots 0 0"
    echo "/dev/${SSD}p2 /var/log btrfs rw,noatime,compress=zstd,space_cache=v2,subvol=@var_log 0 0"
} >> /mnt/etc/fstab
...
echo "1. System will boot directly without encryption"
